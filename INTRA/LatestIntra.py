...
# ---------------------- Global varibales ---------------------- #
file = "WBA.csv"
threeObjectives = True # Set to true if three objectives are to be used.
notification = False
...
pset.renameArguments(ARG0='Datetime')
...
def simulation(individual):
    ...
    startDate = '2021-03-13 08:00:00' # Start date of the overall trading window.
    endDate = '2021-03-26 08:00:00' # End date of the overall trading window.
    ...
    pcSplit = 13 # The number of intervals to split the trading window into for PC
...
toolbox.register("expr_mut", gp.generate_safe, min_=1, max_=3, terminal_types=terminal_types)
...
def threeScatterPlot(allValues):
    df = pd.DataFrame(allValues, columns=['Profit','PC','Risk Exposure'])
    pd.plotting.scatter_matrix(df, alpha=0.2)


def threeDimensionalPlot(inputPoints, dominates):
    paretoPoints = set()
    candidateRowNr = 0
    dominatedPoints = set()
    while True:
...
if __name__ == "__main__":
    trainingStart = "2020-03-13 08:00:00" # Put these dates into the simulation function!
    trainingEnd = "2021-03-26 08:00:00"
    unseenStart = "2021-03-26 08:00:00"
    unseenEnd = "2021-03-26 17:00:00"
    freeze_support()
    random.seed()
    pareto, stats = main(trainingStart,trainingEnd,parallel=False)
    unseen(pareto, unseenStart, unseenEnd)
